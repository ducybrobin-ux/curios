...
Router SPA (`router.js` + `data-go`) × nav multi-fichiers `.html` × hash-router Hub (`#/page`) × boutons topbar concepteur. **Résolu (P2)** : shell nav partagé `js/nav-shell.js` injecté via l'ancre `#cur-nav[data-nav-current]` sur les 6 espaces — **JOUER** (`index.html`) / **PARCOURS** (`catalogue.html`) / **CRÉER** (`atelier.html`, `editeur.html`, `studio.html`) / **PILOTER** (`dashboard.html`) / **⚙ Hub** (`hub/app.html`), lien actif `cur-nav__link--active` + `aria-current`. Barre `.cur-navbar` dans `curios-design-system.css` (adaptative thème). Chemins harmonisés vers `.html` (app.js, topbars, alias serveur `/catalogue`, `/atelier`, `/studio`, `/hub`). Panneau « Administrer » du joueur resserré (Mode admin + Réglages). **Stats réparties** : tableau de bord → PILOTER, carnet/palmarès → JOUER, stats → PARCOURS/CRÉER ; **stats du Hub dashboard câblées** sur les endpoints réels (`js/hub-api.js` + `loadStats` : `/api/hub/analytics` projets/sessions, `/api/packs`, `/api/pos` équipes) au lieu des tirets en dur. Fix dist : `catalogue.html` (`rootFiles`) et dossier `hub/` (`dirs`) absents des distributions → ajoutés au build.
...
- [x] **Consolider les 2 stacks d'auth (API PBKDF2 organisateur + Hub multi-user)** : fait.
  - Nouveau module partagé `packages/shared/src/password.js` (source de vérité unique du hachage) : `hashPassword`, `verifyPassword`, `timingSafeEqualString` + constantes `PBKDF2_ITERATIONS`/`KEY_LENGTH`/`DIGEST`/`SALT_LENGTH` (100 000 it., SHA-512, 64 octets, sel 16 octets).
  - `packages/server/src/auth.js` (organisateur) et `packages/server/src/routes/hub-auth.js` (Hub multi-user) l'importent désormais — plus de copie locale du crypto PBKDF2. La vérification Hub passe de `===` à une comparaison **à temps constant** (amélioration).
  - Nouveau `tests/unit/password-consolidation.test.mjs` (4 tests : round-trip hash/vérif, déterminisme par sel, mauvais mot de passe, constantes). **398/398 tests verts ; eslint 0 erreur.**
...
- [x] **Navigation unifiée** **JOUER / PARCOURS / CRÉER / PILOTER / ⚙** : shell partagé `js/nav-shell.js` (ancre `#cur-nav[data-nav-current]`) injecté sur les 6 espaces, barre `.cur-navbar` (design system), liens `.html` harmonisés (app.js + topbars + alias serveur), panneau « Administrer » restreint à Mode admin/Réglages. **Tableau de bord/carnet/stats répartis** (PILOTER/JOUER/PARCOURS/CRÉER) et **stats Hub câblées** sur `/api/hub/analytics` + `/api/packs` + `/api/pos`. **Branchement `packages/analytics` terminé** : `js/analytics.js` (via `tools/build-analytics.mjs`), hooks player (`BALISE_FOUND`, `RIDDLE_SOLVED`, `QUIZ_COMPLETED`, `RUN_FINISHED` + écrans + indices), tests 21/21, CI synchronisation, SW bump `curios-v9`.
